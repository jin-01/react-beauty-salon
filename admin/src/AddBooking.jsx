...
function AddBooking() {
  ...
  useEffect(() => {
    ...
    const fetchBranchNames = async () => {
      try {
        const response = await axios.get('http://localhost:8088/branches');
  ...
  const handleNameSelect = (selectedName) => {
    if (selectedName === '') {
      setSelectName(false);
      setData({ ...data, name: '', area: '' });
    } else {
      const selectedBranch = branchNames.find(
        (branch) => branch.name === selectedName
      );
      setSelectName(true);
      setData({ ...data, name: selectedName, area: selectedBranch.area });
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchHairstylistNames = async () => {
      try {
        if (data.area) {
          const response = await axios.get(`http://localhost:8088/hairstylists?area=${data.area}`);
  ...
  return (
    <div className='d-flex flex-column align-items-center pt-4'>
      ...
      <form class="row g-3 w-50" onSubmit={handleSubmit}>
        ...
        <div class="col-12">
          <label for="inputPassword4" class="form-label">Cancel Option</label>
        ...
        <div class="col-12">
          ...
          <input type="text" class="form-control" id="inputTime" placeholder='Enter Rating Description'
            onChange={e => setData({ ...data, ratingdesc: e.target.value })} />
        </div>
        <div class="col-12">
          <label for="inputPassword4" class="form-label">Rating</label>
          <input type="text" class="form-control" id="inputTime" placeholder='Enter Rating'
